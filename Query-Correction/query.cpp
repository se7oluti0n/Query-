...
int Cquery::Reference(int xmin,int ymin,int xmax,int ymax,int task)
{
	...
	file.open("./Detected-Object/object.log");
	...
	while(file.getline(buff,80)>0){
		SplitData(buff);
		if((xmin-10<=info[total_obj_num].place.cx) && (info[total_obj_num].place.cx<=xmax+10)
			&& (ymin-10<=info[total_obj_num].place.cy) && (info[total_obj_num].place.cy<=ymax+10)){
	...
	for(int i=0;i<l_num;i++){
		for(int j=0;j<m_num;j++){
			if(LeaveInfo[i].LID==MissingInfo[j].MID){
				LeftInfo[left_num++] = LeaveInfo[i];
				for(int k=i;k<l_num;k++){
					LeaveInfo[k] = LeaveInfo[k+1];
				}
				l_num--;
				j=0;
			}
	...
	for(int i=0;i<left_num;i++){
		cout << i << ":" << LeftInfo[i].frame << "\n";
	}

	cout << "\n持ち去られた物体数：" << m_num << "\n";
	for(int i=0;i<m_num;i++){
		cout << i << ":" << MissingInfo[i].frame << "\n";
	}*/
...
int Cquery::SplitData(char *string)
{
	...
	for(int i=0;i<(int)strlen(string);i++){
		if(string[i]==','){ //','が来たら
			...
			j=0;
			switch(flag){
				case 0:	info[total_obj_num].frame=atoi(value); //フォルダの格納
						break;
				case 1: info[total_obj_num].place.cx=atoi(value); //重心のx座標格納
						break;
				case 2:	info[total_obj_num].place.cy=atoi(value); //重心のy座標格納
						break;
				case 3: info[total_obj_num].place.xmin=atoi(value); //外接長方形(xmin)の格納
						break;
				case 4: info[total_obj_num].place.ymin=atoi(value); //外接長方形(ymin)の格納
						break;
				case 5: info[total_obj_num].place.xmax=atoi(value); //外接長方形(xmax)の格納
						break;
				case 6: info[total_obj_num].place.ymax=atoi(value); //外接長方形(ymax)の格納
						break;
				case 7: info[total_obj_num].area=atoi(value);  //面積の格納
						break;
				case 9: info[total_obj_num].MID=atoi(value);   //持ち去りID
						break;
				case 10: info[total_obj_num].LID=atoi(value);  //持ち込みID
						break;
			}
			flag++;
	...
	value[j]='\0';
	info[total_obj_num].shadow=atoi(value); //影の格納
...
int Cquery::SplitTime(char *string2)
{
	...
	for(int i=0;i<10;i++){
		if(j==2){
			...
			j=0;
			switch(flag){
				case 0: info[total_obj_num].time.month=atoi(value);  //月の格納
						break;
				case 1: info[total_obj_num].time.day=atoi(value);    //日の格納
						break;
				case 2: info[total_obj_num].time.hour=atoi(value);   //時の格納
						break;
				case 3: info[total_obj_num].time.minute=atoi(value); //分の格納
						break;
			}
			i--;
			tmp=0;
	...
	info[total_obj_num].time.second=tmp; //秒の格納
...
void Cquery::Check_Priority()
{
	...
	//シーンの判断
	if(info[total_obj_num].MID<0)	flag=LEAVE;
	else				flag=MISSING;

	//格納
	if(flag==LEAVE)	LeaveInfo[l_num++]=info[total_obj_num];
	else			MissingInfo[m_num++]=info[total_obj_num];
